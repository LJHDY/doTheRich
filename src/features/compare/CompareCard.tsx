...
import { getPriceHistories, getNearbySchools, NearbySchool, getTradeHistory, getTradeHistoryStatus, collectTradeHistory, TradeHistoryMonth } from '../../services/api';
...
const CompareCard: React.FC<CompareCardProps> = ({ complex, onClose }) => {
  ...
  const [tradeSelectedYear, setTradeSelectedYear] = useState<string>('');
  ...
  useEffect(() => {
    ...
    setTradeSelectedYear('');
    getTradeHistoryStatus(complex.id).then(s => {
      if (s.collected) {
        setTradeCollected(true);
        getTradeHistory(complex.id).then(setTradeHistory).catch(() => {});
      }
    }).catch(() => {});
  }, [complex.id]);

  // 거래량 수집 트리거 + 폴링
  const handleCollectTradeHistory = useCallback(async () => {
    ...
    const poll = setInterval(async () => {
      ...
      try {
        ...
        if (s.collected) {
          ...
          setTradeCollected(true);
          setTradeCollecting(false);
          const rows = await getTradeHistory(complex.id);
          setTradeHistory(rows);
        }
      } catch { /* ignore */ }
      if (attempts >= 48) { clearInterval(poll); setTradeCollecting(false); }
    }, 5000);
  }, [complex.id]);
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '12px 14px' }}>
        ...
        {(() => {
            ...
            if (tradeGranularity === 'month') {
              if (showAllMonths) {
                ...
                const monthMap = new Map(
                  tradeHistory.filter(m => m.yearMonth.slice(0, 4) === effectiveYear)
          ...
          const years = tradeGranularity === 'year' ? chartData.length
            ...
            : showAllMonths ? chartData.length / 12
            : 1;
          const annualAvg = years > 0 ? Math.round(totalCount / years) : 0;

          return (
            <div style={{ marginBottom: '12px', borderTop: '1px solid #f0f0f0', paddingTop: '12px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '6px', flexWrap: 'wrap' }}>
                <h4 style={{ fontSize: '13px', fontWeight: 700, color: '#344054', margin: 0 }}>거래량 이력</h4>
                  ...
                  <>
                    ...
                    {tradeGranularity === 'month' && availableYears.length > 0 && (
                      <div style={{ display: 'flex', alignItems: 'center', gap: '2px' }}>
                        <button
                          onClick={() => {
                            if (showAllMonths) { setTradeSelectedYear(latestYear); }
                            else {
                          ...
                          style={{ fontSize: '10px', padding: '1px 4px', border: '1px solid #dadce0', borderRadius: '4px', background: '#fff', cursor: 'pointer', color: '#5f6368' }}
                        >◀</button>
                        <span style={{ fontSize: '11px', fontWeight: 600, color: '#344054', padding: '0 3px', minWidth: '32px', textAlign: 'center' }}>
                        ...
                        <button
                          ...
                          style={{
                            fontSize: '10px', padding: '1px 5px', marginLeft: '1px',
                    ...
                    <button onClick={handleCollectTradeHistory} style={{
                      fontSize: '10px', padding: '1px 6px', border: '1px solid #dadce0',
                ...
                {!tradeCollected && !tradeCollecting && (
                  <button onClick={handleCollectTradeHistory} style={{
                    fontSize: '10px', padding: '1px 7px', border: '1px solid #4BAAD4',
                    borderRadius: '4px', background: '#fff', color: '#4BAAD4', cursor: 'pointer',
                  }}>10년 데이터 수집</button>
                )}
                {tradeCollecting && (
                  <span style={{ fontSize: '10px', color: '#9aa0a6' }}>수집 중…</span>
                )}
              </div>
                ...
                <>
                  ...
                  <ResponsiveContainer width="100%" height={160}>
                    <ComposedChart data={chartData} margin={{ top: 4, right: 8, left: -18, bottom: 0 }}>
                      ...
                      <XAxis
                        dataKey="label"
                        tick={{ fontSize: 8, fill: '#9aa0a6' }}
                        interval={showAllMonths ? 11 : 0}
                        tickFormatter={showAllMonths ? (v: string) => v.slice(0, 4) : undefined}
                      />
                      <YAxis yAxisId="left" orientation="left"
                        tick={{ fontSize: 8, fill: '#89CFF0' }} allowDecimals={false}
                        label={{ value: '건', position: 'insideTopLeft', fontSize: 8, fill: '#89CFF0', dy: -4 }} />
                      <YAxis yAxisId="right" orientation="right"
                        tick={{ fontSize: 8, fill: '#E06060' }}
                        domain={[minPriceY, maxPriceY]}
                        tickFormatter={(v: number) => `${v.toFixed(1)}`}
                        label={{ value: '억', position: 'insideTopRight', fontSize: 8, fill: '#E06060', dy: -4 }} />
                      <Tooltip
                        contentStyle={{ fontSize: '10px' }}
                        formatter={(v: number, name: string) =>
                          name === 'count' ? [`${v}건`, '거래량'] : [`${v != null ? v.toFixed(2) : '-'}억`, '평균가']
                        }
                      />
                      <Bar yAxisId="left" dataKey="count" fill="#89CFF0" maxBarSize={16} radius={[2, 2, 0, 0]} opacity={0.85} />
                      <Line yAxisId="right" dataKey="avgPrice" stroke="#E06060" strokeWidth={1.5} dot={false} connectNulls />
                    </ComposedChart>
                  </ResponsiveContainer>
                  <div style={{ fontSize: '10px', color: '#9aa0a6', marginTop: '4px', display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
